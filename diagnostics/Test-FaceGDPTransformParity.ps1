...
try {
    ...
    foreach ($case in 'neutral','morph') {
        $args = @($gdp, $mmt)
        $referencePrefix = Join-Path $output "$case-x86"
        $nativePrefix = Join-Path $output "$case-x64"
        $referenceArgs = $args + @($referencePrefix)
        $nativeArgs = $args + @($nativePrefix)
        if ($case -eq 'morph') {
            $expression = $Amplitude.ToString('G9', $culture)
            $referenceArgs += @($MacroName, $expression)
            $nativeArgs += @($MacroName, $expression)
        }
        & $x86 @referenceArgs
        if ($LASTEXITCODE -ne 0) { throw "x86 GDP transform failed: $case" }
        & $x64 @nativeArgs
        if ($LASTEXITCODE -ne 0) { throw "x64 GDP transform failed: $case" }
        ...
        if ($left.Count -ne 419 -or $right.Count -ne $left.Count) {
            throw "GDP transform vertex count mismatch: $case"
        }
        $max = 0.0
        $bad = 0
        for ($i = 0; $i -lt $left.Count; ++$i) {
            if ($left[$i].vertex -ne $right[$i].vertex) {
                throw "GDP transform vertex identity mismatch: $case row $i"
            }
            foreach ($axis in 'x','y','z') {
                $a = [double]::Parse($left[$i].$axis, $culture)
                $b = [double]::Parse($right[$i].$axis, $culture)
                if (![double]::IsFinite($a) -or ![double]::IsFinite($b)) {
                    throw "Nonfinite GDP transform coordinate: $case row $i axis $axis"
                }
                $delta = [Math]::Abs($a - $b)
